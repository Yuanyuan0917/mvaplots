...
generate_heatmap_strips <- function(data, outcome_var, strip_vars, variable_labels) {
  ...
  reg_formula <- as.formula(
    paste(outcome_var, "~", paste(strip_vars, collapse = " + "))
  )
  ...
  for (var in strip_vars) {
    ...
    coef_sub  <- tidy_coefs %>%
      dplyr::filter(var == !!var & level != ref_level) %>%
  ...
  # Generate strips
  heatmap_strips <- Map(
    function(varname, base_color) {
      var_label <- variable_labels[[varname]]
      generate_heatmap_strip(
        data, varname, outcome_var, breaks, base_color, var_label,
        label_lookup, common_xlim, x_min, x_max
      )
    },
    varname    = strip_vars,
...
add_variable_label_row <- function(data, varname, base_color, var_label, x_min, x_max) {
  ...
  label_row <- data.frame(
    outcome_bin = NA, prop = NA, xmin = x_min, xmax = x_max, level = 0
  )
  label_row[[varname]] <- factor("label", levels = new_levels)

  for (col in setdiff(names(data), names(label_row))) {
    label_row[[col]] <- NA
  }
