...
generate_heatmap_strips <- function(data, outcome_var, strip_vars, variable_labels) {
  ...
  heatmap_strips <- Map(function(varname, base_color) {
    ...
    generate_heatmap_strip(data, varname, outcome_var,
                           breaks, base_color, var_label,
...
#' @keywords internal
generate_heatmap_strip <- function(df, varname, outcome_var,
                                   breaks, base_color, var_label,
                                   label_lookup, common_xlim, x_min, x_max) {
  ...
  label_data <- heatmap_data %>%
    dplyr::filter(!is.na(!!var_sym), !is.na(prop), !!var_sym != "label") %>%
  ...
  ggplot2::ggplot(heatmap_data, ggplot2::aes(y = !!var_sym, fill = prop)) +
    ...
    ggplot2::geom_text(data = label_data,
                       ...
                       hjust = 0, size = 3, parse = TRUE, na.rm = TRUE) +
